...
pub struct Unigram {
    token_to_ids: TokenMap,                 // token string -> vocab id
    pub(crate) vocab: Vocab,                // (token, score) where score ~ log-prob
    cache: Cache<String, Vec<String>>,      // small LRU cache for encode results
    trie: Trie<u8>,                         // byte-wise prefix trie for fast prefix matches
    pub min_score: f64,                     // min score across vocab (used for unk penalty)
    pub(super) unk_id: Option<usize>,       // optional <unk> id in vocab
    pub(super) bos_id: usize,               // BOS local lattice id (not in vocab)
    pub(super) eos_id: usize,               // EOS local lattice id (not in vocab)

    fuse_unk: bool,                         // if true, consecutive UNKs are merged
    is_optimized: bool,                     // use fast DP path vs lattice path
...
impl PartialEq for Unigram {
    fn eq(&self, other: &Self) -> bool {
        // equality considers unk_id and the vocab contents
...
impl Default for Unigram {
    fn default() -> Self {
        // default model contains only <unk>
...
impl Unigram {
    ...
    /// Returns whether the model is in unit-cost mode (each token costs -1.0).
    pub fn unit_cost(&self) -> bool {
        self.unit_cost
    }

    /// Enables/disables unit-cost mode. Also clears the encode cache to avoid stale entries.
    ...
    /// Populate a lattice with all tokens that start at each position.
    /// This is the standard (unfiltered) version used by the classic unigram trainer.
    pub(super) fn populate_nodes(&self, lattice: &mut Lattice) {
        ...
        while begin_pos < len {
                ...
                .common_prefix_search(lattice.sentence.bytes().skip(begin_pos))
            {
                ...
                let id_u32 = *self.token_to_ids.get(&tok).unwrap();
                let id: usize = id_u32.try_into().unwrap();
            ...
            // NOTE:
            // - In standard mode, we may insert UNK to ensure coverage.
            // - In unit_cost mode we typically guarantee Σ is in the vocab, so UNK isn't needed.
            if !has_single_node {
    ...
    /// NEW: Populate a lattice but only insert nodes for token ids for which `allow(id)` is true.
    /// This lets a trainer evaluate "V \\ {t}" or "V minus disabled set" with no trie rebuild.
    pub(super) fn populate_nodes_filtered<F>(&self, lattice: &mut Lattice, mut allow: F)
    where
        F: FnMut(usize) -> bool,
    {
        let unk_score = self.min_score - K_UNK_PENALTY;
        ...
        while begin_pos < len {
                ...
                .common_prefix_search(lattice.sentence.bytes().skip(begin_pos))
            {
                ...
                let tok = String::from_utf8(bytes).unwrap();
                let id_u32 = *self.token_to_ids.get(&tok).unwrap();
                let id: usize = id_u32 as usize;

                if !allow(id) {
                    continue; // skip filtered-out tokens
                }
            ...
            // Keep UNK insertion only in non-unit-cost mode for safety/compat with old behavior
            if !has_single_node {
                if let Some(unk_id) = self.unk_id {
                    if !self.unit_cost {
                        lattice.insert(begin_pos, mblen, unk_score, unk_id);
    ...
    fn encode_optimized(&self, sentence: &str) -> Result<Vec<String>> {
        ...
        while starts_at < size {
            ...
            if !has_single_node {
                // NOTE:
                // In unit-cost training we typically ensure Σ exists, so this branch should be rare.
                // Keep UNK insertion only for non-unit-cost mode for compatibility/safety.
                if !self.unit_cost {
                    ...
                    // Unit-cost mode but no token matched: this implies Σ wasn't present.
                    // As a defensive fallback, if unk_id exists, use it; else break (cannot proceed).
                    if let Some(unk) = self.unk_id {
                        ...
                        // No valid token; advance to avoid infinite loop (acts like a hard UNK)
                        let target_node = &mut best_path_ends_at[starts_at + mblen];
                        target_node.best_path_score = best_path_score_till_here - 1.0; // one byte cost
                        target_node.starts_at = Some(starts_at);
                        target_node.id = 0; // dummy; will be ignored by backtracking if unreachable
                    }
                }
            }
            starts_at += mblen
    ...
    fn encode_unoptimized(&self, sentence: &str) -> Result<Vec<String>> {
        // Lattice path (kept for parity/testing); respects unit_cost via populate_nodes
    ...
    /// Iterate over vocabulary of the model as a pair of `(token, score)`.
